 ...
 * 
 * 【核心设计思想】不直接存储绝对时间，而是存储“PTS相对于系统时间的漂移量”。
 * 这样即便没有新帧到来，get() 也能通过系统时间流逝推算出当前应处的播放位置。
 * ========================================== */
class PlayerClock {
public:
    PlayerClock() {
    ...
    /**
     * 设置当前时钟值（通常在新帧到来或 Seek 跳转时调用）
     * @param new_pts    新的 PTS（秒）
     * @param new_serial 流序列号（用于丢弃 Seek 前的旧缓存数据）
     * 
     * 【关键逻辑】锚定时钟锚点：
     * 记录“此刻的系统时间”，并算出“PTS 与系统时间的差值（漂移）”。
     * 以后只要系统时间还在走，我们就能用这个漂移量推算任意时刻的 PTS。
     */
    void set(double new_pts, int new_serial) {
        // av_gettime_relative() 返回单调递增的系统时间（不受系统时间调整影响）
        double time = av_gettime_relative() / 1000000.0;  
        
        pts = new_pts;
        last_updated = time;                 // 锚定时刻的系统时间
        pts_drift = new_pts - time;          // 漂移量 = 想达到的PTS - 当前系统时间
        serial = new_serial;
    }

    /**
     * 获取当前时钟值（考虑流逝的时间，若暂停则返回固定 PTS）
     * @return 当前时钟值（秒），若从未 set 过则返回 NAN
     * 
     * 【数学推导】完整计算公式：
     * result = pts_drift + time - (time - last_updated) * (1.0 - speed)
     * 
     * 1. 当 speed = 1.0（正常播放）：
     *    公式退化为 pts_drift + time，即“系统时间 + 固定偏移”，完全跟随物理时间。
     * 
     * 2. 当 speed = 2.0（2倍速快进）：
     *    公式变为 pts_drift + time + (time - last_updated)。
     *    即“上次更新后流逝的每一秒，在时钟里要算作 2 秒”，实现时间加速。
     * 
     * 3. 当 speed = 0.5（0.5倍慢放）：
     *    公式变为 pts_drift + time - 0.5 * (time - last_updated)。
     *    即“上次更新后流逝的每一秒，在时钟里只算 0.5 秒”，实现时间减速。
     */
    double get() {
        // 暂停状态下时钟完全冻结，始终返回上次设置的固定 PTS
        if (paused) return pts;
        
        // 如果从未初始化过（pts 无效），则返回无效值
        if (std::isnan(pts)) return NAN;
        
        double time = av_gettime_relative() / 1000000.0;
        
        // 核心计算：pts_drift 是基准偏移，后面的修正项负责处理倍速带来的时间缩放
     ...
     * 
     * 【恢复时的关键操作】：
     * 暂停期间系统时间依然在飞奔（比如暂停了 10 秒）。
     * 如果恢复时不重新锚定，get() 会瞬间跳过 10 秒的内容（画面跳跃）。
     * 因此恢复时立即调用 set(get(), serial)：
     *   1. get() 返回暂停那一刻的固定 PTS（比如 10.5 秒）
     *   2. set() 用“当前系统时间”和“10.5 秒”重新计算 pts_drift
     *   3. 此后时钟从 10.5 秒接着走，完美衔接，无跳跃。
     */
    void set_paused(bool p) {
        if (paused != p) {
            paused = p;
            if (!p) {  // 从暂停恢复
                // 传入当前 paused 状态下的 pts（即冻结时的那一帧时间）和当前系统时间重新锚定
                set(get(), serial);
            }
        }
    }
    ...
    /**
     * 主从同步（解决时钟跑飞）
     * @param slave 从时钟（例如外部时钟作为主，音视频时钟作为从）
     * 
     * 【调用场景】当同步模式为 AV_SYNC_EXTERNAL_CLOCK 时：
     * 外部时钟（主）每隔一段时间调用此函数，传入音/视频时钟（从）。
     * 
     * 【执行逻辑】如果当前时钟（主时钟）无效，或者主时钟与从时钟的偏差 
     * 超过了 AV_NOSYNC_THRESHOLD（10秒，定义在全局常量中），
     * 说明主时钟由于异常跑飞了，这时强制放弃主时钟自己的值，直接追齐从时钟。
     * 这避免了音画彻底不同步导致的糟糕体验。
     */
    void sync_to_slave(PlayerClock& slave) {
        double slave_time = slave.get();   // 从时钟的值（可靠参考）
        double master_time = get();        // 当前主时钟的值
        
        // 若主时钟无效，或主从差异超过了“放弃同步”的极限阈值（10秒）
        if (!std::isnan(slave_time) && (std::isnan(master_time) || std::fabs(master_time - slave_time) > AV_NOSYNC_THRESHOLD)) {
            // 强制将主时钟的值设置为从时钟的值，并使用从时钟的序列号（丢弃旧数据）
...
public:
    ...
    double pts;            // 当前最新的 PTS 值（秒），作为锚定基准点
    double pts_drift;      // 核心漂移量：PTS - 锚定时的系统时间，决定了时钟曲线的截距
    double last_updated;   // 最近一次调用 set() 时的系统时间（秒），用于计算时间增量
    double speed;          // 播放倍速（1.0 正常，>1 快进，<1 慢放）
    int serial;            // 流序列号。每次 Seek 或换流时自增，set() 时会带上该序号，
                           // 配合外部逻辑丢弃 serial 不匹配的旧帧，防止旧数据污染新时钟
    bool paused;           // 暂停标志位（true 时 get() 固定返回 pts）
};
